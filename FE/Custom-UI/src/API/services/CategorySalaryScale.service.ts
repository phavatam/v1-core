...
export const CategorySalaryScaleApisService = createApi({
  ...
  endpoints: (builder) => ({
    ...
    UpdateCategorySalaryScale: builder.mutation<
      payloadResult,
      { CategorySalaryScale: Partial<CategorySalaryScaleDTO> }
    >({
    ...
    GetCategorySalaryScaleById: builder.query<ListResponse<CategorySalaryScaleDTO>, { idCategorySalaryScale: string }>({
      ...
      providesTags: (result, error, arg) => [{ type: "CategorySalaryScaleApisService", id: arg.idCategorySalaryScale }]
    }),
    InsertCategorySalaryScale: builder.mutation<
      payloadResult,
      { CategorySalaryScale: Partial<CategorySalaryScaleDTO> }
    >({
      query: ({ CategorySalaryScale }) => ({
