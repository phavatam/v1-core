...
export const EmployeeApisService = createApi({
  ...
  endpoints: (builder) => ({
    ...
    GetListEmployeeByIdUnit: builder.query<ListResponse<EmployeeDTO>, { idUnit: string; pagination: pagination }>({
      query: ({ idUnit, pagination }): any => ({
        url: `/employee/getListEmployeeByIdUnit?idUnit=${idUnit}&pageSize=${pagination.pageSize}&pageNumber=${pagination.pageNumber}`
      }),
      providesTags(result) {
        if (result && result.listPayload) {
          const { listPayload } = result;
...
export const {
  ...
  useGetListEmployeeByIdUnitQuery
} = EmployeeApisService;
