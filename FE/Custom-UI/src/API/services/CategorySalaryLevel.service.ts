...
export const CategorySalaryLevelApisService = createApi({
  ...
  endpoints: (builder) => ({
    ...
    UpdateCategorySalaryLevel: builder.mutation<
      payloadResult,
      { CategorySalaryLevel: Partial<CategorySalaryLevelDTO> }
    >({
    ...
    GetCategorySalaryLevelById: builder.query<ListResponse<CategorySalaryLevelDTO>, { idCategorySalaryLevel: string }>({
      ...
      providesTags: (result, error, arg) => [{ type: "CategorySalaryLevelApisService", id: arg.idCategorySalaryLevel }]
    }),
    InsertCategorySalaryLevel: builder.mutation<
      payloadResult,
      { CategorySalaryLevel: Partial<CategorySalaryLevelDTO> }
    >({
      query: ({ CategorySalaryLevel }) => ({
