...
import { ListResponse, pagination, payloadResult, CoreResponse } from "~/models/common";
...
export const NavigationApisService = createApi({
  ...
  endpoints: (builder) => ({
    ...
    GetListNavigationByToken: builder.query<CoreResponse<NavigationDTO>, pagination>({
      query: ({ pageSize, pageNumber }): any => ({
        url: `/navigation/get-navigation`
      })
    }),
