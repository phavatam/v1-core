...
import type { NavigationDTO, Navigation } from "@models/navigationDTO";

type Map = {
  [key: string]: React.JSX.Element[];
};
export const DataNavigate = () => {
const { data: result } = useGetListNavigationByTokenQuery({ pageNumber: 0, pageSize: 0 });

  // const listNavigation = {
  //   listPayload: [
  //     {
  //       navigation: {
  //         id: "019A2A3B-0472-4AD6-9D4E-C00A992EFF89",
  //         menuName: "Danh mục xếp loại",
  //       },
  //       navigationsChild: [
  //       {
  //           id: "019A2A3B-0472-4AD6-9D4E-C00A992EFF99",
  ...
  //       },
  //       {
  //         id: "6E0FD8F9-7BDC-423B-AE7F-CB06E4D4F1C7",
  ...
  const mapNavigation = result?.data?.map((items: NavigationDTO) => {
    return {
      ...items,
      navigationsChild: items?.navigationsChild?.map((x: Navigation) => {
        return {
          ...x,
