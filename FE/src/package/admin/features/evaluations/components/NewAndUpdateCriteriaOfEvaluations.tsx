...
interface IProps {
  ...
  refetchDetailsAndSortingEvaluationsCriteria?: (idEvaluations: string) => void;
}

export function arrayToTree(menuItems: CategoryCriteriaDTO[]) {
...
function _NewAndUpdateCriteriaOfEvaluations(props: IProps) {
  const { setVisible, idEvaluations, refetchDetailsAndSortingEvaluationsCriteria } = props;
  const [checkStrictly, setCheckStrictly] = useState(true);

  const {
    data: ListEvaluationsCriteriaByIdEvaluations,
    isLoading: LoadingListEvaluationsCriteriaByIdEvaluations,
    refetch: refetchListEvaluationsCriteriaByIdEvaluations
  } = useGetListEvaluationsCriteriaByIdEvaluationsQuery(
    {
  ...
  const getAllIds = (data: CategoryCriteriaDTO[]): any[] => {
    let ids: any[] = [];
    data.forEach((item) => {
      ids.push(item.id);
      if (item.children) {
  ...
  const getSelectedKeys = (data: CategoryCriteriaDTO[]): any[] => {
    const dbIds = ListEvaluationsCriteriaByIdEvaluations?.listPayload?.map((d) => d.idCategoryCriteria) ?? [];
    const selectedKeys: Key[] = [];
    const allIds = getAllIds(data);
    allIds.forEach((id) => {
      if (dbIds && dbIds.includes(id)) {
  ...
  useEffect(() => {
    if (ListEvaluationsCriteriaByIdEvaluations?.listPayload && ListCategoryCriteria?.listPayload) {
      ...
      setSelectedRowKeys(getSelectedKeys(arrayTreeCategoryCriteria));
    }
  }, [ListEvaluationsCriteriaByIdEvaluations, ListCategoryCriteria, idEvaluations]);
  ...
  const onfinish = async () => {
    try {
      ...
      if (result.success) {
        ...
        if (refetchDetailsAndSortingEvaluationsCriteria) {
  ...
  const onReload = async () => {
    await refetchListEvaluationsCriteriaByIdEvaluations();
    await refetchListCategoryCriteria();
    const arrayTreeCategoryCriteria = arrayToTree(ListCategoryCriteria?.listPayload ?? []);
    const selectedKeys = getSelectedKeys(arrayTreeCategoryCriteria);
    setSelectedRowKeys(selectedKeys);
    setCheckStrictly(true);
  };
  ...
  return (
    <div className="NewAndUpdateCriteriaOfEvaluations">
      <Spin
        spinning={
          LoadingListEvaluationsCriteriaByIdEvaluations ||
          LoadingListCategoryCriteria ||
          LoadingListTypeAssessment ||
          LoadingListUnit
        }
      >
        <Row gutter={16}>
...
export const NewAndUpdateCriteriaOfEvaluations = WithErrorBoundaryCustom(_NewAndUpdateCriteriaOfEvaluations);
