...
function _CategorySalaryScale() {
  const { data: ListCategorySalaryScale, isLoading: LoadingListCategorySalaryScale } =
    useGetListCategorySalaryScaleQuery({
      pageSize: 0,
      pageNumber: 0
    });
  const [deleteListCategorySalaryScale, { isLoading: isLoadingDeleteListCategorySalaryScale }] =
    ...
    <div className={"unit"}>
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Danh mục ngạch lương </Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
          ...
          >
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  const result = await deleteListCategorySalaryScale({
                    idCategorySalaryScale: selectedRowKeys as string[]
                  }).unwrap();
