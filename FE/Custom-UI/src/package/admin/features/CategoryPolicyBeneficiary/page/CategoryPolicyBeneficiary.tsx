...
const _CategoryPolicyBeneficiary = () => {
  const { data: ListCategoryPolicyBeneficiary, isLoading: LoadingCategoryPolicyBeneficiary } =
    useGetListCategoryPolicyBeneficiaryQuery({
      pageSize: 0,
      pageNumber: 0
    });
  const [deleteListCategoryPolicyBeneficiary, { isLoading: isLoadingDeleteListCategoryPolicyBeneficiary }] =
    useDeleteCategoryPolicyBeneficiaryMutation();
    ...
    <div className={"unit"}>
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Danh mục đối tượng chính sách </Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
          ...
          >
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  const result = await deleteListCategoryPolicyBeneficiary({
                    idCategoryPolicyBeneficiary: selectedRowKeys as string[]
                  }).unwrap();
