...
function _CategoryEducationalDegree() {
  const { data: ListCategoryEducationalDegree, isLoading: LoadingListCategoryEducationalDegree } =
    useGetListCategoryEducationalDegreeQuery({
      pageSize: 0,
      pageNumber: 0
    });
  const [deleteListCategoryEducationalDegree, { isLoading: isLoadingDeleteListCategoryEducationalDegree }] =
    ...
    <div className={"unit"}>
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Danh mục bằng cấp </Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
          ...
          >
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  const result = await deleteListCategoryEducationalDegree({
                    idCategoryEducationalDegree: selectedRowKeys as string[]
                  }).unwrap();
