...
import { NewAndUpdateCategoryJobStatus } from "@admin/features/categoryJobStatus";
...
function _CategoryJobStatus() {
  const { data: ListCategoryJobStatus, isLoading: LoadingListCategoryJobStatus } = useGetListCategoryJobStatusQuery({
    pageSize: 0,
    pageNumber: 0
  });
  const [deleteListCategoryJobStatus, { isLoading: isLoadingDeleteListCategoryJobStatus }] =
    useDeleteCategoryJobStatusMutation();
  ...
  // const handleChange: TableProps<CategoryJobStatusDTO>["onChange"] = (pagination, filters) => {
  ...
  const menuAction = (record: CategoryJobStatusDTO) => {
    return (
      <Menu>
        <Menu.Item
          key="1"
  ...
  const columns: ColumnsType<CategoryJobStatusDTO> = [
    {
      title: "Tên trạng thái công việc",
      dataIndex: "jobStatusName",
      key: "jobStatusName"
      // filters: ListCategoryJobStatus?.listPayload?.map((item) => ({
  ...
  const propsTable: TableProps<CategoryJobStatusDTO> = {
    scroll: {
      x: 800
    },
    bordered: true,
    ...
    dataSource: ListCategoryJobStatus?.listPayload,
    // onChange: handleChange,
    loading: LoadingListCategoryJobStatus,
    pagination: {
      total: ListCategoryJobStatus?.totalElement,
      pageSize: 10,
  ...
  return (
    <div className={"unit"}>
      <ModalContent
        ...
        width={"600px"}
      >
        <NewAndUpdateCategoryJobStatus setVisible={setIsOpenModal} id={id} />
      </ModalContent>
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Danh mục trạng thái công việc </Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
          ...
          >
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  const result = await deleteListCategoryJobStatus({
                    idCategoryJobStatus: selectedRowKeys as string[]
                  }).unwrap();
            ...
            >
              <Button
                danger
                type="primary"
                loading={isLoadingDeleteListCategoryJobStatus}
                disabled={!(selectedRowKeys.length > 0)}
                icon={<DeleteOutlined />}
              >
...
export const CategoryJobStatus = WithErrorBoundaryCustom(_CategoryJobStatus);
